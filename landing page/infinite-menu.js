// 3D Infinite Menu - Converted from React to Vanilla JS
// Uses gl-matrix library (already loaded in HTML)

(function() {
    'use strict';
    
    // Ensure glMatrix is loaded
    if (typeof glMatrix === 'undefined') {
        console.error('glMatrix library not loaded! Make sure to include gl-matrix before this script.');
        return;
    }
...
`;
    ...
    class InfiniteGridMenu {
        ...
        #init(onInit) {
            ...
            const gl = this.gl;
            if (!gl) {
                throw new Error('No WebGL 2 context! Your browser may not support WebGL 2.0.');
            }
        ...
        #initTexture() {
            ...
            const colors = [
                ['#667eea', '#764ba2'], // Personal - Purple gradient
                ['#f093fb', '#f5576c'], // Academic - Pink gradient
                ['#DDA0DD', '#BA55D3'], // Sports - Light purple gradient
                ['#87CEEB', '#4facfe']  // Activities - Sky blue gradient
            ];
            ...
            Promise.all(
                this.items.map((item, index) =>
                    new Promise(resolve => {
                        if (item.image) {
                            ...
                            img.onload = () => resolve({ img, index, type: 'image' });
                            img.onerror = () => {
                                // If image fails to load, create gradient
                                resolve({ index, type: 'gradient', colors: colors[index % colors.length] });
                            };
                            img.src = item.image;
                        } else {
                            // No image provided, use gradient
                            resolve({ index, type: 'gradient', colors: colors[index % colors.length] });
                        }
                    })
                )
                ...
                results.forEach(result => {
                    const x = (result.index % this.atlasSize) * cellSize;
                    const y = Math.floor(result.index / this.atlasSize) * cellSize;
                    ...
                    if (result.type === 'image') {
                        ...
                        // Add title text
                        const item = this.items[result.index];
    ...
    console.log('✅ InfiniteGridMenu loaded successfully!');
